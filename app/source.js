...
async function actuallyUploadAnimation(
  ...
  if (!resp.ok) {
    throw new Error(
      `Animation upload failed (status ${resp.status}): ${await resp.text()}`
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    https
      .get(url, { headers: { Cookie: cookie } }, (res) => {
        console.log(
          `[downloadAssetLegacy] Response for assetId ${assetId}: ${res.statusCode}`
        );
        if (res.statusCode !== 200) {
          ...
          return reject(
            new Error(
              `Download failed (status ${res.statusCode}) for assetId ${assetId}`
            )
          );
        }
        ...
        if (encoding === "gzip") {
          console.log(
            "[downloadAssetLegacy] Content-Encoding: gzip -> zlib Gunzip"
          );
          stream = res.pipe(zlib.createGunzip());
        } else if (encoding === "deflate") {
          console.log(
            "[downloadAssetLegacy] Content-Encoding: deflate -> zlib Inflate"
          );
        ...
        fileStream.on("finish", () => {
          fileStream.close(() => {
            console.log(
              `[downloadAssetLegacy] Finished writing file: ${outputPath}`
            );
            resolve();
          });
        });
  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (
        ...
        console.warn(
          `[downloadAssetLegacyWithRetries] 429 or rate-limit for asset ${assetId}, attempt ${attempt}`
        );
        if (attempt < maxRetries) {
          await sleep(1000);
        } else {
...
async function pollOperationUntilDone(operationId, apiKey) {
  ...
  while (true) {
    ...
    if (data.done === true) {
      ...
      if (!finalAssetId) {
        throw new Error(
          `Operation done but no assetId. Full data: ${JSON.stringify(data)}`
        );
      }
      return finalAssetId;
    }
...
async function createImageAsset(
  ...
  if (response.status === 201) {
    ...
    if (!data.assetId) {
      throw new Error(
        `Response missing assetId. Full data: ${JSON.stringify(data)}`
      );
    }
    const rawModeration =
      data.moderationResult || data.response?.moderationResult;
  ...
  if (response.status === 200) {
    ...
    if (!opData.operationId) {
      throw new Error(
        `Got 200 but no operationId. Full data: ${JSON.stringify(opData)}`
      );
    }
    const finalAssetId = await pollOperationUntilDone(
      opData.operationId,
      apiKey
    );
...
function prepareOutputDir(dir) {
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      console.warn(
        `[prepareOutputDir] Attempt ${attempt} failed: ${err.message}`
      );
      if (attempt < 5) {
        Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 200);
      } else {
        console.error(
          "[prepareOutputDir] Gave up trying to create output folder."
        );
      }
    }
  }
...
async function runJobImages(jobId, assetIDs, creatorID, isGroup, apiKey, assetNames) {
  ...
  try {
    console.log(
      `[runJobImages] Starting job ${jobId} with ${assetIDs.length} assets.`
    );
    ...
    for (const rbxAssetIdStr of assetIDs) {
      ...
      // Handle both regular IDs and unique keys (format: "rbxassetid://123#1")
      let actualId = rbxAssetIdStr;
      if (assetNames && assetNames[rbxAssetIdStr]?.originalId) {
        actualId = assetNames[rbxAssetIdStr].originalId;
      }

      const match = actualId.match(/\d+/);
      if (!match) {
      ...
      const fileName = `asset_${numericId}_${downloadedCount}.png`;
      const filePath = path.join(outputDir, fileName);

      try {
        await downloadAssetLegacyWithRetries(numericId, filePath);
        downloaded.push({ fileName, oldId: rbxAssetIdStr, originalId: actualId });
        jobInfo.message = `${downloadedCount}/${assetIDs.length} downloaded`;
      } catch (err) {
        console.error(
          `[runJobImages] Download error for ${rbxAssetIdStr}:`,
          err
        );
        jobInfo.failures.push({
          assetId: rbxAssetIdStr,
    ...
    for (let i = 0; i < downloaded.length; i += 60) {
      ...
      if (i + 60 < downloaded.length) {
        ...
        console.log(
          "[runJobImages] Upload+moderation batch done. Sleeping 60s..."
        );
        await sleep(60_000);
      }
    }
...
async function runJobAnimations(
  jobId,
  assetIDs,
  creatorID,
  isGroup,
  ...
  try {
    console.log(
      `[runJobAnimations] Starting job ${jobId} with ${assetIDs.length} animations.`
    );
    prepareOutputDir(outputDir);
    ...
    if (!cookie) {
      console.log(
        "[runJobAnimations] No cookie in JSON => calling rbx_cookie.exe..."
      );
      ...
      cookie = autoCookie;
      console.log(
        `[runJobAnimations] Retrieved cookie from Rust CLI: ${cookie}`
      );
    }
    ...
    for (let i = 0; i < assetIDs.length; i += 60) {
      ...
      for (const rbxAssetIdStr of slice) {
        ...
        const numericId = match[0];
        const ext = ".xml";
        const fileName = `asset_${numericId}_${downloadedCount}${ext}`;
        const filePath = path.join(outputDir, fileName);
        ...
        try {
          ...
          console.error(
            `[runJobAnimations] Download error for ${rbxAssetIdStr}:`,
            err
          );
          jobInfo.failures.push({
            assetId: rbxAssetIdStr,
    ...
    for (let i = 0; i < downloaded.length; i += 60) {
      ...
      for (const item of slice) {
        ...
        try {
          ...
          const displayName = customName || path.basename(filePath);
          const desc = `Reuploaded from ${displayId}`;
          ...
          console.error(
            `[runJobAnimations] Upload error for ${item.oldId}:`,
            err
          );
          jobInfo.failures.push({
            assetId: item.oldId,
...
async function uploadMesh(filePath, cookie, creatorID, isGroup, customName) {
  const baseUrl = "https://data.roblox.com/ide/publish/UploadNewMesh";
  const url = new URL(baseUrl);
  const meshName = customName || "RenderMesh";
  url.searchParams.set("name", meshName);
  url.searchParams.set("description", meshName);
  url.searchParams.set("isPublic", "False");
...
async function runJobMeshes(jobId, assetIDs, creatorID, isGroup, apiKey, assetNames) {
  const jobInfo = jobs[jobId];
  if (!jobInfo) {
    throw new Error(`No job info found for jobId ${jobId}`);
  }
  ...
  try {
    console.log(
      `[runJobMeshes] Starting job ${jobId} with ${assetIDs.length} meshes.`
    );
    prepareOutputDir(outputDir);
    ...
    for (const rbxAssetIdStr of assetIDs) {
      ...
      const numericId = match[0];
      const fileName = `asset_${numericId}_${downloadedCount}.rbxmesh`;
      const filePath = path.join(outputDir, fileName);

      try {
        await downloadAssetLegacyWithRetries(numericId, filePath);
        downloaded.push({ filePath, oldId: rbxAssetIdStr, originalId: actualId });
        jobInfo.message = `${downloadedCount}/${assetIDs.length} downloaded`;
      } catch (err) {
        console.error(
          `[runJobMeshes] Download error for ${rbxAssetIdStr}:`,
          err
        );
        jobInfo.failures.push({
          assetId: rbxAssetIdStr,
...
function handleUpload(req, res, body) {
  ...
  const { assetIDs, creatorID, isGroup, apiKey, uploadType, cookie, assetNames } =
    payload || {};
  ...
  if (uploadType === "Animations") {
    ...
    runJobImages(jobId, assetIDs, creatorID, !!isGroup, apiKey, assetNames || null).catch((err) => {
      console.error(`[handleUpload] Fatal image job error:`, err);
      jobs[jobId].message = `Fatal: ${err.message}`;
      jobs[jobId].finished = true;
    });
  } else if (uploadType === "Meshes") {
    runJobMeshes(jobId, assetIDs, creatorID, !!isGroup, apiKey, assetNames || null).catch((err) => {
