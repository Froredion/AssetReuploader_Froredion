...
local ReplacingOrderImages =
	{ "ShirtTemplate", "PantsTemplate", "Texture", "Image", "TextureID", "TextureId", "ColorMap" }
	...
	SearchPromise = Promise.new(function(resolve, reject)
		...
		local function run(checkingInstance)
			...
			if assetTypeAnimation then
				...
				-- Explicit ParticleEmitter support
				if checkingInstance:IsA("ParticleEmitter") and ReplacingImages.Texture:get() then
					local success, texture = pcall(function()
						return checkingInstance.Texture
					end)
					if success and texture then
						foundProperty = "Texture"
						foundID = texture
					end
				end
				...
				if foundID:match("%S") then
					...
					if idNumber then
						...
						if success and productInfo then
							...
							local creatorTypeMatches = (GroupOwner:get() and creatorType == "Group")
								or (not GroupOwner:get() and creatorType == "User")
...
local function CreateBackupFolder(foundInstances)
	-- Find the top folder that contains all found instances
	if #foundInstances == 0 then
		return nil
	end

	-- Start with the first instance and find its top-level ancestor
	local firstInstance = foundInstances[1].instance
	local topFolder = nil

	-- Find the highest ancestor under game that's a folder or model
	local current = firstInstance
	while current and current.Parent do
		if current.Parent == game or current.Parent:IsA("DataModel") then
			-- We've reached a top-level service
			-- Now find the first folder/model child
			local checkParent = firstInstance
			while checkParent and checkParent.Parent ~= current do
				if checkParent.Parent == current then
					topFolder = checkParent
					break
				end
		...
		if current.Parent and (current.Parent:IsA("Folder") or current.Parent:IsA("Model")) then
			topFolder = current.Parent
		end
		current = current.Parent
	end

	-- If we still haven't found a suitable folder, use the first non-service ancestor
	if not topFolder then
		current = firstInstance
		while current and current.Parent do
			local parent = current.Parent
			if parent == game or parent:IsA("DataModel") or parent:IsA("ServiceProvider") then
				topFolder = current
				break
			end
			current = parent
	...
	if not topFolder then
		warn("Could not find top folder to backup")
		return nil
	end

	-- Create a backup with timestamp
	local timestamp = os.date("%Y%m%d_%H%M%S")
	local backupName = topFolder.Name .. "_Backup_" .. timestamp

	local backup = topFolder:Clone()
	backup.Name = backupName
	backup.Parent = topFolder.Parent
...
local function RunServer(params)
	...
	-- Store these in outer scope so they're accessible in the :andThen callback
	local instanceToKeyMap = params.instanceToKeyMap
	...
	local ServerPromise = Promise.new(function(resolve, reject)
		...
		local assetNames = params.assetNames -- New: mapping of asset IDs to names
		...
		-- Add asset names if provided
		if assetNames then
			uploadData.assetNames = assetNames
		end

		local success, response = pcall(function()
			return HttpService:PostAsync(
				"http://localhost:3000/upload",
				HttpService:JSONEncode(uploadData),
		...
		if resultsCount > 0 then
			...
			while offset < resultsCount do
				...
				local chunkUrl = ("http://localhost:3000/chunks?jobId=%s&offset=%d&count=%d"):format(
					jobId,
					offset,
					count
				)
		...
		if moderatedCount > 0 then
			...
			while offset < moderatedCount do
				local count = chunkSize
				local modUrl = ("http://localhost:3000/moderated?jobId=%s&offset=%d&count=%d"):format(
					jobId,
					offset,
					count
				)
		...
		print("All chunks retrieved. Approved results: " .. #finalResults .. ", moderated: " .. #finalModerated)
	...
	ServerPromise:andThen(function(resultTables)
		...
		if resultTables then
			...
			for _, instEntry in foundInstances do
				...
				local lookupKey = id
				if instanceToKeyMap and instanceToKeyMap[instance] then
					lookupKey = instanceToKeyMap[instance]
				end
...
local function CreateFusionUi(widget)
	...
	for i = 1, #Groups do
		...
		template.TextLabel.TextColor3 = if settings().Studio.Theme.Name == "Dark"
	...
	local CreateBackupBox = Checkbox({
		Parent = SearchSettingsList,
		Text = "Create Backup",
		Value = CreateBackup,
		Enabled = NotRunning,
	})
	local UseInstanceNamesBox = Checkbox({
		Parent = SearchSettingsList,
		Text = "Use Instance Names",
		Value = UseInstanceNames,
		Enabled = NotRunning,
	})
	...
	local RunButton = Button({
		...
		Activated = function()
			...
			if #foundInstances > 0 then
				...
				if CreateBackup:get() then
					local backup = CreateBackupFolder(foundInstances)
					if backup then
						print("Backup created successfully: " .. backup:GetFullName())
					else
						warn("Failed to create backup")
					end
				end

				-- Collect asset IDs and optionally their names
				local assetIDs = {}
				...
				if UseInstanceNames:get() then
					...
					-- Default behavior: deduplicate by asset ID
					for i, entry in foundInstances do
						local id = entry.id
				...
				RunServer({
					...
					instanceToKeyMap = instanceToKeyMap,
