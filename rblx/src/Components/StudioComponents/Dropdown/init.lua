...
return function(props: DropdownProperties): Frame
	...
	local disconnect = Observer(isOpen):onChange(function()
		local openVal = isOpen:get()
		DropdownFrame:SetAttribute("IsOpen",openVal)
	end)
	...
	local newDropdown = Hydrate(DropdownFrame){
		Name = "Dropdown",
